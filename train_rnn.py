...
for epoch in xrange(start_epoch, config.max_epoch):
    ...
    for train_batch_idxs in train_data_iterator:
        ...
        train_loss, predict_clas = train(x_batch, mask_batch)
        ...
        if predict_clas == x_batch:
            correctT += 1
        prev_time = current_time
        losses_train.append(train_loss)
        niter += 1
        #if niter % config.validate_every == 0:
        if niter % float(train_batches_per_epoch) == 0:
            print 'Validating'
            avg_valid_loss = 0
            contadorV = 0
            for valid_batch_idx in valid_data_iterator:
                ...
                avg_validloss, predict_clasV = validate(x_batch, mask_batch)
                avg_valid_loss += avg_validloss
                if predict_clasV == x_batch:
                    correctV += 1
            HaccuracyT.append(float(correctT / contadorV))
            avg_valid_loss /= nvalid_batches
            losses_eval_valid.append(avg_valid_loss)
            losTrain.append(train_loss)
            print "    loss:\t%.6f" % avg_valid_loss
            print
            print "  saved to %s" % metadata_target_path
    HaccuracyT.append(float(correctT / contadorT))
...
plt.subplot(211)
plt.plot(losses_eval_valid)
plt.title('model loss Normal')
plt.ylabel('loss')
plt.xlabel('epoch')
